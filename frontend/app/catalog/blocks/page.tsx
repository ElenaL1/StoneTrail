...
import { stoneBlocks } from "@/lib/mock-data"
...
export default function BlocksPage() {
  ...
  const allStoneTypes = useMemo(
    () => Array.from(new Set(stoneBlocks.map((b) => b.stoneType))),
    [],
  )

  const totalBlocksInCatalog = useMemo(
    () => stoneBlocks.reduce((s, b) => s + getBlockCount(b.blocks), 0),
    [],
  )

  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase()
    return stoneBlocks.filter((b) => {
          ...
          <div className="relative grid gap-4 items-end p-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span>
                ...
                {filtered.length !== stoneBlocks.length
