...
import { featuredMaterials, stoneBlocks } from "@/lib/mock-data"
import {
  getBlockCount,
...
export default function BlockDetailPage() {
  ...
  const block = stoneBlocks.find((b) => b.slug === slug)

  // state for lightbox: `null` = closed, `n` = open at index n
  ...
  const related = useMemo(() => {
    ...
    return stoneBlocks
      ...
      .sort((a, b) => {
        const score = (x: (typeof stoneBlocks)[0]) =>
  ...
  const relatedMaterial = useMemo(() => {
    if (!block?.blockStoneId) return null
    return (
      featuredMaterials.find((m) => m.id === block.blockStoneId) ?? null
    )
  }, [block])

  if (!block) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 px-5 py-24">
        <p className="text-xl text-muted-foreground">Блок не найден</p>
        <Button asChild variant="outline" className="h-10">
          <Link href="/catalog/blocks">Вернуться к каталогу блоков</Link>
        </Button>
      </div>
    )
  }
